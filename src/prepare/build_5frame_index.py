...
OUTPUT_CSV = "/home/saslab01/Desktop/replay_pad/frame_index/replayattack_5frame_index.csv"

WINDOW_SIZE = 5
STRIDE = 1


def build_sequence_index(df, window_size=5, stride=1):
    rows = []
...
def main():
    ...
    if len(out_df) == 0:
        raise ValueError("[ERROR] No 5-frame sequences were created")

    out_df = out_df.sort_values(["split", "video_id", "start_frame_idx"]).reset_index(drop=True)
    out_df.to_csv(OUTPUT_CSV, index=False)

    print(f"[INFO] Saved 5-frame index -> {OUTPUT_CSV}")
