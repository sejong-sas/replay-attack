...
CKPT_PATH = ROOT / "outputs" / "checkpoints" / "resnet18_1frame_best.pth"

DEVEL_PRED_CSV = ROOT / "outputs" / "predictions" / "resnet18_devel_frame_predictions.csv"
TEST_PRED_CSV = ROOT / "outputs" / "predictions" / "resnet18_test_frame_predictions.csv"
DEVEL_VIDEO_CSV = ROOT / "outputs" / "predictions" / "resnet18_devel_video_predictions.csv"
TEST_VIDEO_CSV = ROOT / "outputs" / "predictions" / "resnet18_test_video_predictions.csv"

RESULT_JSON = ROOT / "outputs" / "results" / "resnet18_eval_results.json"
FRAME_SUBGROUP_CSV = ROOT / "outputs" / "results" / "resnet18_test_frame_subgroups.csv"
VIDEO_SUBGROUP_CSV = ROOT / "outputs" / "results" / "resnet18_test_video_subgroups.csv"
...
def collect_predictions(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            logits = model(images)
            probs = F.softmax(logits, dim=1)[:, 1].cpu().numpy()  # spoof prob
...
def main():
    ...
    model = ResNet18BinaryClassifier(pretrained=PRETRAINED).to(device)
    ...
    print(f"[INFO] devel frame predictions saved -> {DEVEL_PRED_CSV}")
    print(f"[INFO] test frame predictions saved -> {TEST_PRED_CSV}")

    # threshold on devel frame-level
    ...
    results = {
        ...
        "paths": {
            "devel_frame_predictions": str(DEVEL_PRED_CSV),
            "test_frame_predictions": str(TEST_PRED_CSV),
            "devel_video_predictions": str(DEVEL_VIDEO_CSV),
            "test_video_predictions": str(TEST_VIDEO_CSV),
            "frame_subgroups": str(FRAME_SUBGROUP_CSV),
            "video_subgroups": str(VIDEO_SUBGROUP_CSV),
        }
    }
    ...
    print(f"[INFO] frame subgroup csv saved -> {FRAME_SUBGROUP_CSV}")
    print(f"[INFO] video subgroup csv saved -> {VIDEO_SUBGROUP_CSV}")
